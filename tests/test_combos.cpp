...
{
    check(
        hexDigest(sha256(std::string("abc"))) ==
    ...
    for (auto pack : {PackAlgorithm::Stream, PackAlgorithm::Index})
    {
              ...
              CompressionAlgorithm::Huffman})
        {
                  ...
                  EncryptionAlgorithm::Aes256})
            {
                ...
                fs::path archive =
                    workspace / ("combo-" + std::to_string(number) + ".bak");
                ...
                auto created = BackupEngine::create(source.string(),
...
{
    auto found = std::find_if(archives.begin(), archives.end(),
                              [&](const auto& archive)
                              {
                                  return archive.pack == pack &&
