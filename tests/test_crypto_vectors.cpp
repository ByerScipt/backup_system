...
void testCryptoVectors()
{
    ...
    check(
        toHex(zeros) ==
            "8adc91fd9ff4f0f51b0fad50ff15d637e40efda206cc52c783a74200503c15"
    ...
    check(std::vector<uint8_t>(zeros.begin() + 5, zeros.begin() + 105) ==
              window,
          "chacha20 stream offset failed");

    std::string text = "Ladies and Gentlemen of the class of '99: If I could "
                       "offer you only one tip "
    ...
    check(
        toHex(message) ==
            "c6bdf594fa87d094756b8d179a7ba25b816398cc26a334e7f7cf2720335074f1"
    ...
    detail::aes256CtrXor(key,
                         fromHexToArray16("00112233445566778899aabbccddeeff"),
                         0, fips.data(), fips.size());
    check(toHex(fips) == "8ea2b7ca516745bfeafc49904b496089",
          "aes-256 FIPS C.3 vector failed");
    ...
    detail::aes256CtrXor(key, zeroCounter, 7, ctrWindow.data(),
                         ctrWindow.size());
