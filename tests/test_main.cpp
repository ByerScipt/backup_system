...
void testNetwork(const fs::path& workspace, const fs::path& archive) {
    ...
    try{
        std::string error;network::BackupClient alice("127.0.0.1",port,"alice","alice-password");check(alice.registerUser(error),"Alice registration failed: "+error);error.clear();check(!alice.registerUser(error),"duplicate registration was accepted");error.clear();std::string id;check(alice.upload(archive.string(),"integration backup",id,error),"network upload failed: "+error);check(id.size()==32,"server returned invalid backup ID");error.clear();auto entries=alice.list(error);check(error.empty()&&entries.size()==1&&entries[0].id==id,"network list mismatch: "+error);fs::path downloaded=workspace/"downloaded.bak";check(alice.download(id,downloaded.string(),error),"network download failed: "+error);check(sha256File(downloaded.string())==sha256File(archive.string()),"network roundtrip checksum mismatch");fs::path brokenDownload=workspace/"broken-download.bak";check(::symlink("missing-download",brokenDownload.c_str())==0,"broken download symlink fixture failed");error.clear();check(!alice.download(id,brokenDownload.string(),error),"network download overwrote a broken symlink");check(fs::is_symlink(brokenDownload),"network download did not preserve broken symlink");struct stat storageState{};check(stat((workspace/"server-data"/"users.db").c_str(),&storageState)==0&&(storageState.st_mode&0777)==0600,"user database permissions are not 0600");fs::path aliceDir=workspace/"server-data"/"users"/hexDigest(sha256(std::string("alice")));check(stat(aliceDir.c_str(),&storageState)==0&&(storageState.st_mode&0777)==0700,"user storage directory permissions are not 0700");check(stat((aliceDir/(id+".bak")).c_str(),&storageState)==0&&(storageState.st_mode&0777)==0600,"stored archive permissions are not 0600");check(stat((aliceDir/(id+".meta")).c_str(),&storageState)==0&&(storageState.st_mode&0777)==0600,"backup metadata permissions are not 0600");
        ...
        fs::path cancelledUpload=workspace/"cancelled-upload.bin";writeBytes(cancelledUpload,std::vector<uint8_t>(2*1024*1024,0x5a));std::atomic_bool cancel{false};error.clear();std::string cancelledId;auto cancelAfterChunk=[&](const ProgressEvent& event){if(event.completed>=512*1024)cancel.store(true);};check(!alice.upload(cancelledUpload.string(),"cancelled",cancelledId,error,cancelAfterChunk,&cancel),"cancelled upload unexpectedly completed");check(error.find("cancelled")!=std::string::npos,"cancelled upload returned the wrong error");std::this_thread::sleep_for(std::chrono::milliseconds(100));error.clear();entries=alice.list(error);check(error.empty()&&entries.size()==1,"cancelled upload left a visible half-finished backup");
