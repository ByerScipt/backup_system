...
{
    std::string pattern =
        (fs::temp_directory_path() / "backup-tests-XXXXXX").string();
...
void makeSocketNode(const fs::path& path)
{
    ...
    check(path.string().size() < sizeof(address.sun_path),
          "socket fixture path too long");
    ...
    check(false,
          "socket fixture bind failed: " + std::string(std::strerror(error)));
}
...
void createFixture(const fs::path& root)
{
    ...
    writeBytes(root / "executable.sh",
               {'#', '!', '/', 'b', 'i', 'n', '/', 's', 'h', '\n'});
    check(::chmod((root / "executable.sh").c_str(), 0751) == 0,
    ...
    writeBytes(root / "nested" / "single-symbol.bin",
               std::vector<uint8_t>(4096, 0x41));
    ...
    writeBytes(deep / (std::string(180, 'x') + ".bin"),
               {'l', 'o', 'n', 'g', '\n'});

    check(::chmod((root / "hello.txt").c_str(), 0600) == 0,
          "file mode fixture failed");
    timespec executableTimes[2]{{1700000000, 123456789},
                                {1700000001, 987654321}};
    check(::utimensat(AT_FDCWD, (root / "executable.sh").c_str(),
                      executableTimes, 0) == 0,
          "timestamp fixture failed");
    check(::symlink("../hello.txt", (root / "nested" / "hello-link").c_str()) ==
              0,
          "symlink fixture failed");
    check(::mkfifo((root / "named-pipe").c_str(), 0640) == 0,
          "FIFO fixture failed");
...
std::vector<std::string> listTree(const fs::path& root)
{
    ...
    for (fs::recursive_directory_iterator
             it(root, fs::directory_options::none, error),
         end;
                     ...
                     const std::vector<std::string>& paths)
{
    for (size_t index = 0; index < paths.size(); ++index)
    {
        ...
        check(::lstat(path.c_str(), &state) == 0,
              "cannot stat fixture for atime normalization");
                               ...
                               const std::vector<std::string>& expectedPaths)
{
    ...
    for (const std::string& relative : expectedPaths)
    {
        ...
        check(::lstat(path.c_str(), &state) == 0,
              "tree entry is missing: " + relative);
        TreeEntry entry;
        ...
        entry.size =
            S_ISREG(state.st_mode) ? static_cast<uint64_t>(state.st_size) : 0;
        ...
        if (S_ISLNK(state.st_mode))
        {
            ...
            ssize_t length =
                ::readlink(path.c_str(), target.data(), target.size());
            ...
            check(::utimensat(AT_FDCWD, path.c_str(), preserve,
                         ...
                         const TreeSnapshot& expected)
{
    ...
    check(actual.size() == expected.size(),
          "restored tree entry count mismatch");
    for (const auto& [path, source] : expected)
    {
        ...
        check(target.content == source.content,
              "entry content mismatch: " + path);
        check(target.linkTarget == source.linkTarget,
              "link target mismatch: " + path);
        check(sameTime(target.atime, source.atime),
              "nanosecond atime mismatch: " + path);
        check(sameTime(target.mtime, source.mtime),
              "nanosecond mtime mismatch: " + path);
    }
}
