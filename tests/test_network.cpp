...
void testServerConfig(const fs::path& workspace)
{
    ...
    for (const std::string setting :
         {"port = 65537", "port = -65535", "port = 8848junk",
          "max_connections = 4294967297", "timeout_seconds = -1"})
    {
        ...
        check(rejected,
              "invalid server configuration was accepted: " + setting);
    }
    std::ofstream(path)
        << "port = 65535\nmax_connections = 1\ntimeout_seconds = 3\n";
    auto config = network::ServerConfig::load(path.string());
    check(config.port == 65535 && config.maxConnections == 1 &&
...
void testLoginRequestId(uint16_t port)
{
    ...
    protocol::sendFrame(socket.get(), protocol::MessageType::LoginStart, 10,
                        payload);
    ...
    auto proof = protocol::proofFor(
        protocol::verifierFor("alice-password", salt), nonce);
...
void testNetwork(const fs::path& workspace, const fs::path& archive)
{
    ...
    try
    {
        std::string error = "previous operation failed";
        network::BackupClient alice("127.0.0.1", port, "alice",
                                    "alice-password");
        ...
        check(error.empty(), "successful download retained a stale error");
        ...
        check(::stat((workspace / "server-data" / "users.db").c_str(),
                     &state) == 0 &&
        ...
        if (::geteuid() != 0)
        {
            check(::chmod(aliceDirectory.c_str(), 0000) == 0,
                  "cannot restrict user storage fixture");
            error.clear();
            const auto inaccessible = alice.list(error);
            check(::chmod(aliceDirectory.c_str(), 0700) == 0,
                  "cannot unlock user storage fixture");
            check(inaccessible.empty() && !error.empty(),
                  "unreadable user storage was reported as an empty successful "
                  "list");
        }
        else
        {
            std::cout
                << "SKIP: unreadable user storage requires a non-root user\n";
        }

        network::BackupClient wrong("127.0.0.1", port, "alice", "wrong");
        error.clear();
        auto denied = wrong.list(error);
        check(denied.empty() && !error.empty(),
              "wrong account password was accepted");
        network::BackupClient bob("127.0.0.1", port, "bob", "bob-password");
        error.clear();
        check(bob.registerUser(error), "Bob registration failed: " + error);
        error.clear();
        check(bob.list(error).empty() && error.empty(),
              "cross-user list isolation failed");
        error.clear();
        check(
            !bob.download(id, (workspace / "bob-download.bak").string(), error),
            "cross-user download was accepted");

        fs::path cancelledUpload = workspace / "cancelled-upload.bin";
        writeBytes(cancelledUpload,
                   std::vector<uint8_t>(2 * 1024 * 1024, 0x5a));
        ...
        error.clear();
        check(!alice.upload(cancelledUpload.string(), "cancelled", cancelledId,
                            error, cancelAfterChunk, &cancel),
    ...
    std::thread restartThread(
        [&] { check(restarted.run(), "restarted server run failed"); });
    std::this_thread::sleep_for(std::chrono::milliseconds(200));
    try
    {
        std::string error;
