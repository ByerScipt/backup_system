                          ...
                          const std::string& expectedMessage = {})
{
    ...
    check(!result.success,
          "malformed archive was restored: " + archive.filename().string());
                         ...
                         const std::vector<BuiltArchive>& archives)
{
    for (auto compression :
         {CompressionAlgorithm::Rle, CompressionAlgorithm::Huffman})
    {
        const auto& source =
            findArchive(archives, PackAlgorithm::Stream, compression,
        ...
        fs::path malformed =
            workspace / (compression == CompressionAlgorithm::Rle
                             ? "rle-over-output.bak"
    ...
    const auto& huffman =
        findArchive(archives, PackAlgorithm::Stream,
    ...
    emptyArchive.insert(emptyArchive.end(), emptyPayload.begin(),
                        emptyPayload.end());
    writeLe64(emptyArchive, 16, 0);
    writeLe64(emptyArchive, 24, emptyPayload.size());
    auto emptyDigest = sha256(std::vector<uint8_t>{});
    std::copy(emptyDigest.begin(), emptyDigest.end(),
              emptyArchive.begin() + 48);
...
{
    const auto& source =
        findArchive(archives, PackAlgorithm::Index, CompressionAlgorithm::None,
                    EncryptionAlgorithm::None);
    std::vector<uint8_t> bytes = readBytes(source.path);
...
{
    const auto& plain =
        findArchive(archives, PackAlgorithm::Stream, CompressionAlgorithm::None,
                    EncryptionAlgorithm::None);
    ...
    check(!conflictResult.success,
          "late target conflict was overwritten by default");
    ...
    race.progress = [&](const ProgressEvent& event)
    {
        ...
        {
            fs::path parent =
                symlinkDestination / source.filename() / "中文目录";
            check(::rmdir(parent.c_str()) == 0,
                  "cannot remove parent race fixture");
            check(::symlink(outside.c_str(), parent.c_str()) == 0,
                  "cannot install parent race symlink");
            symlinkInjected = true;
        }
    };
    auto raceResult = BackupEngine::restore(plain.path.string(),
                                            symlinkDestination.string(), race);
    check(symlinkInjected, "parent symlink race fixture was not injected");
    check(!raceResult.success,
          "symlink parent race escaped restore destination");
    check(!fs::exists(outside / "数据.txt"),
          "restore wrote through raced symlink parent");
}
                      ...
                      const std::vector<BuiltArchive>& archives)
{
    ...
    auto wrongResult =
        BackupEngine::restore(encrypted.path.string(),
    ...
    auto first =
        BackupEngine::restore(plain.path.string(), conflict.string(), {});
    ...
    auto third = BackupEngine::restore(plain.path.string(), conflict.string(),
                                       overwrite);
    check(third.success, "explicit overwrite failed: " + third.message);
    auto preview =
        BackupEngine::preview(plain.path.string(), conflict.string());
    ...
    writeBytes(truncated,
               std::vector<uint8_t>(bytes.begin(), bytes.begin() + 50));
    rejected = false;
    try
    {
    ...
    auto mutationResult = BackupEngine::create(
        mutationSource.string(), (workspace / "mutation.bak").string(),
        mutationOptions);
    ...
    check(!fs::exists(workspace / "evil"),
          "path traversal escaped destination");
    ...
    check(original.size() == replacement.size(),
          "duplicate fixture lengths differ");
    auto duplicateAt = std::search(duplicate.begin() + 112, duplicate.end(),
                                   original.begin(), original.end());
    check(duplicateAt != duplicate.end(),
          "cannot locate duplicate path fixture");
    ...
    rootAt = std::search(oversized.begin() + 112, oversized.end(), root.begin(),
                         root.end());
    ...
    check(regularPath.size() == childPath.size(),
          "symlink path fixture lengths differ");
    auto regularAt =
        std::search(symlinkEscape.begin() + 112, symlinkEscape.end(),
                    regularPath.begin(), regularPath.end());
    check(regularAt != symlinkEscape.end(),
          "cannot locate symlink child fixture");
    ...
    check(targetAt != symlinkEscape.end(),
          "cannot locate symlink target fixture");
    ...
    check(!fs::exists(workspace / "out"),
          "archive symlink escaped destination");
    ...
    expectRestoreFailure(trailingArchive,
                         workspace / "huffman-trailing-target");
