...
void configureFileDialog(QFileDialog& dialog, QWidget* owner, bool directoryMode) {
    ...
    const QSize available =
        targetScreen ? targetScreen->availableGeometry().size() : QSize(1280, 800);
    ...
    const int targetWidth =
        std::min(maximumWidth, std::clamp(qRound(available.width() * 0.78), 860, 1280));
    const int targetHeight =
        std::min(maximumHeight, std::clamp(qRound(available.height() * 0.74), 560, 840));

    dialog.setSizeGripEnabled(true);
    dialog.setMinimumSize(std::min(760, targetWidth), std::min(500, targetHeight));
    ...
    const QString capturePath = qEnvironmentVariable("BACKUP_GUI_DIALOG_CAPTURE");
