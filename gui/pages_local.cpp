...
QWidget* localBackupPage() {
    Page page = makePage();
    Card configuration = makeCard("备份配置", "选择来源、输出位置以及归档处理策略。");
    auto* form = makeForm();
    ...
    QObject::connect(browseOutput, &QPushButton::clicked, page.widget, [=]() {
        const QString value =
            selectArchiveOutput(page.widget, "Select Output Archive", output->text());
        if (!value.isEmpty())
    ...
    QObject::connect(controls.start, &QPushButton::clicked, page.widget, [=]() {
        ...
        if (src.isEmpty() || out.isEmpty()) {
            QMessageBox::warning(page.widget, "Input Error", "请选择源目录和输出归档。");
            return;
        }
        if (QFile::exists(out)) {
        ...
        startJob(
            page.widget, controls, [=](std::atomic_bool* cancel, const ProgressCallback& progress) {
                ...
                auto result = BackupEngine::create(src.toStdString(), out.toStdString(), options);
                if (!result.success) {
...
QWidget* localRestorePage() {
    ...
    form->addRow("目标目录", pathRow(destination, browseDestination, "还原到该目录下"));
    ...
    QObject::connect(browseArchive, &QPushButton::clicked, page.widget, [=]() {
        const QString value = selectArchive(page.widget, "Select Backup Archive", archive->text());
        if (!value.isEmpty())
            archive->setText(value);
    });
    QObject::connect(browseDestination, &QPushButton::clicked, page.widget, [=]() {
        const QString value =
    ...
    QObject::connect(previewButton, &QPushButton::clicked, page.widget, [=]() {
        ...
        if (input.isEmpty() || dest.isEmpty()) {
            QMessageBox::warning(page.widget, "Input Error", "请选择备份归档和目标目录。");
            return;
        }
        ...
        startJob(
            ...
            [=](std::atomic_bool*, const ProgressCallback&) {
                *previewResult = BackupEngine::preview(input.toStdString(), dest.toStdString(),
            ...
            [=]() {
                ...
                if (previewResult->conflicts.empty()) {
                    ...
                    lines << QString("发现 %1 个冲突：").arg(previewResult->conflicts.size());
    ...
    QObject::connect(controls.start, &QPushButton::clicked, page.widget, [=]() {
        ...
        startJob(
            page.widget, controls, [=](std::atomic_bool* cancel, const ProgressCallback& progress) {
                auto resultPreview = BackupEngine::preview(input.toStdString(), dest.toStdString(),
                                                           password.toStdString());
                if (!resultPreview.conflicts.empty()) {
                    QStringList lines;
                    for (size_t i = 0; i < std::min<size_t>(resultPreview.conflicts.size(), 20);
                         ++i) {
                        lines << QString::fromStdString(resultPreview.conflicts[i]);
                    }
                    progress({"conflict-preview", 0, 0, lines.join("；").toStdString()});
                    if (!allowOverwrite) {
                        throw std::runtime_error("目标存在 " +
                ...
                auto result =
                    BackupEngine::restore(input.toStdString(), dest.toStdString(), options);
                if (!result.success) {
                    throw std::runtime_error(result.message);
                }
