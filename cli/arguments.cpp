    ...
    for (int i = start; i < argc; ++i)
    {
        ...
        if (value == "--overwrite")
        {
            result.flags.push_back(value);
                 ...
                 value == "--compress" || value == "--encrypt" ||
                 value == "--key-file" || value == "--server" ||
                 value == "--username" || value == "--account-password-file" ||
                 value == "--name")
        {
            if (i + 1 >= argc)
            {
...
{
    return std::find(args.flags.begin(), args.flags.end(), flag) !=
           args.flags.end();
}
...
std::string promptSecret(const std::string& prompt)
{
    ...
    termios oldState{};
    bool hidden =
        isatty(STDIN_FILENO) && tcgetattr(STDIN_FILENO, &oldState) == 0;
    if (hidden)
    {
                       ...
                       const std::string& key, const std::string& prompt)
{
    ...
    return it == options.end() ? promptSecret(prompt)
                               : readFileSecret(it->second);
}
