...
void progressReport(const ProgressEvent& event)
{
    ...
    if (event.total)
    {
        std::cerr << event.completed << "/" << event.total;
    }
    else
    {
        std::cerr << event.completed;
    }
...
BackupOptions backupOptions(const Arguments& args)
{
    ...
    options.pack = parsePackAlgorithm(
        pack == args.options.end() ? "stream" : pack->second);
    options.compression = parseCompressionAlgorithm(
        compression == args.options.end() ? "none" : compression->second);
    options.encryption = parseEncryptionAlgorithm(
        encryption == args.options.end() ? "none" : encryption->second);
...
{
    std::string pattern =
        (fs::temp_directory_path() / "backup-cli-XXXXXX").string();
    std::vector<char> buffer(pattern.begin(), pattern.end());
    buffer.push_back('\0');
    int fd = mkstemp(buffer.data());
    if (fd < 0)
    {
        throw std::runtime_error("cannot allocate temporary archive path");
    }
    close(fd);
    unlink(buffer.data());
    return buffer.data();
...
int localBackup(const Arguments& args)
{
    ...
    {
        throw std::runtime_error("backup requires one SOURCE directory");
    }
    auto result = BackupEngine::create(args.positional[0],
                                       requiredOption(args.options, "-o"),
                                       backupOptions(args));
    std::cerr << '\n';
    if (!result.success)
    {
    ...
    std::cout << "Backup complete: " << result.entryCount << " entries, "
...
int localRestore(const Arguments& args)
{
    ...
    std::cout << "Restore complete: " << result.entryCount << " entries, "
              << result.outputBytes << " file bytes\n";
    return 0;
}
...
int inspectArchive(const Arguments& args)
{
    if (args.positional.size() != 1)
    {
        throw std::runtime_error("inspect requires one ARCHIVE");
    }
...
network::BackupClient remoteClient(const Arguments& args)
{
    ...
    std::string accountPassword = secretFrom(
        args.options, "--account-password-file", "Account password: ");
...
int remoteBackup(const Arguments& args)
{
    if (args.positional.size() != 1)
    {
        throw std::runtime_error("remote-backup requires one SOURCE directory");
    }
    std::string temp = temporaryArchive();
    RemoveOnExit cleanup{temp};
    auto local =
        BackupEngine::create(args.positional[0], temp, backupOptions(args));
    std::cerr << '\n';
    if (!local.success)
    {
    ...
    auto client = remoteClient(args);
    std::string id, error;
    auto name = args.options.find("--name");
    if (!client.upload(temp, name == args.options.end() ? "" : name->second, id,
                       error))
    {
        throw std::runtime_error(error);
    }
    std::cout << "Remote backup complete. ID: " << id << "\n";
    return 0;
}
...
int remoteList(const Arguments& args)
{
    auto client = remoteClient(args);
    std::string error;
    auto entries = client.list(error);
    if (!error.empty())
    {
        throw std::runtime_error(error);
    }
    std::cout << "Remote backups (" << entries.size() << "):\n";
    for (const auto& e : entries)
    {
        std::cout << e.id << "\t" << e.name << "\t" << e.size << " bytes\t"
                  << e.timestamp << "\n";
    }
    return 0;
}
...
int remoteRestore(const Arguments& args)
{
    ...
    auto client = remoteClient(args);
    std::string error;
    if (!client.download(args.positional[0], temp, error))
    {
        throw std::runtime_error(error);
    }
    ...
    std::cout << "Remote restore complete: " << restored.entryCount
              << " entries\n";
    return 0;
}
