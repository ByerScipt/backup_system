...
BackupOptions backupOptions(const Arguments& args) {
    ...
    options.compression =
        parseCompressionAlgorithm(compression == args.options.end() ? "none" : compression->second);
    options.encryption =
        parseEncryptionAlgorithm(encryption == args.options.end() ? "none" : encryption->second);
...
int localBackup(const Arguments& args) {
    ...
    auto result = BackupEngine::create(args.positional[0], requiredOption(args.options, "-o"),
                                       backupOptions(args));
    std::cerr << '\n';
    if (!result.success)
        throw std::runtime_error(result.message);
    std::cout << "Backup complete: " << result.entryCount << " entries, " << result.outputBytes
              << " bytes\n";
    return 0;
}
...
int localRestore(const Arguments& args) {
    ...
    if (info.encryption != EncryptionAlgorithm::None)
        options.password = secretFrom(args.options, "--key-file", "Archive key: ");
    auto result =
        BackupEngine::restore(args.positional[0], requiredOption(args.options, "-d"), options);
    std::cerr << '\n';
    if (!result.success)
        throw std::runtime_error(result.message);
    std::cout << "Restore complete: " << result.entryCount << " entries, " << result.outputBytes
              << " file bytes\n";
    return 0;
}
...
network::BackupClient remoteClient(const Arguments& args) {
    ...
    std::string accountPassword =
        secretFrom(args.options, "--account-password-file", "Account password: ");
...
int remoteBackup(const Arguments& args) {
    ...
    auto local = BackupEngine::create(args.positional[0], temp, backupOptions(args));
    std::cerr << '\n';
    if (!local.success)
    ...
    if (!client.upload(temp, name == args.options.end() ? "" : name->second, id, error))
        throw std::runtime_error(error);
...
int remoteList(const Arguments& args) {
    ...
    for (const auto& e : entries)
        std::cout << e.id << "\t" << e.name << "\t" << e.size << " bytes\t" << e.timestamp << "\n";
    return 0;
}
