...
bool BackupClient::upload(const std::string& archivePath,const std::string& displayName,
    ...
    try{Socket socket=connectTo(host_,port_);uint32_t request=0;ensure(authenticateClient(socket.get(),request,username_,password_,error),error);uint64_t size=static_cast<uint64_t>(fs::file_size(archivePath));auto digest=sha256File(archivePath);std::vector<uint8_t> start;putString(start,displayName);putU64(start,size);start.insert(start.end(),digest.begin(),digest.end());sendFrame(socket.get(),MessageType::UploadStart,++request,start);auto ready=receiveFrame(socket.get());ensure(ready.has_value(),"server closed before upload");if(ready->type==MessageType::Error)throw NetError(errorFromFrame(*ready));ensure(ready->type==MessageType::UploadReady&&ready->requestId==request,"unexpected upload response");Reader rr(ready->payload);backupId=rr.string();rr.end();std::ifstream in(archivePath,std::ios::binary);ensure(in.good(),"cannot open archive for upload");std::vector<uint8_t> chunk(kChunkSize);while(in){in.read(reinterpret_cast<char*>(chunk.data()),static_cast<std::streamsize>(chunk.size()));size_t got=static_cast<size_t>(in.gcount());if(got)sendFrame(socket.get(),MessageType::UploadChunk,request,{chunk.begin(),chunk.begin()+static_cast<ptrdiff_t>(got)});}sendFrame(socket.get(),MessageType::UploadEnd,request);auto result=receiveFrame(socket.get());ensure(result.has_value(),"server closed after upload");if(result->type==MessageType::Error)throw NetError(errorFromFrame(*result));ensure(result->type==MessageType::UploadResult,"unexpected upload completion response");Reader done(result->payload);backupId=done.string();done.end();return true;}catch(const std::exception& e){error=e.what();return false;}
}
...
bool BackupClient::download(const std::string& backupId,const std::string& outputPath,
                            std::string& error) {
    try{ensure(validBackupId(backupId),"invalid backup identifier");ensure(!fs::exists(outputPath),"download output already exists");Socket socket=connectTo(host_,port_);uint32_t request=0;ensure(authenticateClient(socket.get(),request,username_,password_,error),error);std::vector<uint8_t> payload;putString(payload,backupId);sendFrame(socket.get(),MessageType::DownloadRequest,++request,payload);auto start=receiveFrame(socket.get());ensure(start.has_value(),"server closed before download");if(start->type==MessageType::Error)throw NetError(errorFromFrame(*start));ensure(start->type==MessageType::DownloadStart&&start->requestId==request,"unexpected download response");Reader r(start->payload);uint64_t declaredSize=r.u64();auto digestBytes=r.bytes(32);r.end();std::array<uint8_t,32> declaredDigest{};std::copy(digestBytes.begin(),digestBytes.end(),declaredDigest.begin());fs::path output=fs::absolute(outputPath);fs::create_directories(output.parent_path());fs::path temp=output.string()+".tmp-"+randomId();struct Cleanup{fs::path p;bool keep=false;~Cleanup(){if(!keep){std::error_code ec;fs::remove(p,ec);}}}cleanup{temp};std::ofstream out(temp,std::ios::binary|std::ios::trunc);ensure(out.good(),"cannot create download output");chmod(temp.c_str(),0600);uint64_t received=0;while(true){auto frame=receiveFrame(socket.get());ensure(frame.has_value(),"server disconnected during download");ensure(frame->requestId==request,"download request identifier changed");if(frame->type==MessageType::DownloadEnd)break;ensure(frame->type==MessageType::DownloadChunk,"unexpected download message");ensure(frame->payload.size()<=declaredSize-received,"download exceeds declared size");if(!frame->payload.empty())out.write(reinterpret_cast<const char*>(frame->payload.data()),static_cast<std::streamsize>(frame->payload.size()));ensure(out.good(),"cannot write download output");received+=frame->payload.size();}out.close();ensure(received==declaredSize,"downloaded size mismatch");ensure(sha256File(temp.string())==declaredDigest,"downloaded SHA-256 mismatch");std::error_code ec;fs::rename(temp,output,ec);ensure(!ec,"cannot commit downloaded archive");cleanup.keep=true;return true;}catch(const std::exception& e){error=e.what();return false;}
}
