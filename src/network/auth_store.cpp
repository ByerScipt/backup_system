...
std::array<uint8_t, 32> verifierFor(const std::string& password,
...
std::vector<UserRecord> loadUsers(const fs::path& storage)
{
    ...
    uint32_t count = static_cast<uint32_t>(b[0]) |
...
void saveUsers(const fs::path& storage, const std::vector<UserRecord>& users)
{
    ...
    ensure(chmod(temp.c_str(), 0600) == 0,
           "cannot secure user database permissions");
    out.write("USR1", 4);
    ...
    for (const auto& u : users)
    {
        ...
        out.write(reinterpret_cast<const char*>(u.verifier.data()),
                  u.verifier.size());
    }
    out.close();
...
void writeMeta(const fs::path& path, const RemoteBackupEntry& entry)
{
    ...
    std::ofstream out(temp, std::ios::binary | std::ios::trunc);
           ...
           "cannot secure backup metadata permissions");
    out.write("MET1", 4);
    ...
    little64(entry.size);
    out.write(entry.digest.data(),
              static_cast<std::streamsize>(entry.digest.size()));
    out.close();
...
std::vector<RemoteBackupEntry> listEntries(const fs::path& storage,
                                           const std::string& username)
{
    ...
    ensure(!ec, "cannot inspect user backups: " + ec.message());
    if (!exists)
    {
        return entries;
    }
    fs::directory_iterator it(dir, ec), end;
    ensure(!ec, "cannot enumerate user backups: " + ec.message());
    ...
    ensure(!ec, "cannot finish enumerating user backups: " + ec.message());
    std::sort(entries.begin(), entries.end(), [](const auto& a, const auto& b)
              { return a.timestamp > b.timestamp; });
    return entries;
}

std::optional<UserRecord> findUser(const fs::path& storage,
