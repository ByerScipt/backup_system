                                  ...
                                  const BackupOptions& options)
{
    ...
    try
    {
        ...
        fs::path outputParent = output.parent_path().empty()
                                    ? fs::current_path()
                                    : output.parent_path();
        std::error_code ec;
        ...
        detail::ensure(!detail::isPathInside(canonicalOutput, source),
                       "archive output cannot be inside the source directory");
        ...
        if (options.pack == PackAlgorithm::Stream)
        {
            detail::packStream(entries, packed.path(), inputBytes, options);
        }
        else if (options.pack == PackAlgorithm::Index)
        {
            detail::packIndex(entries, packed.path(), inputBytes, options);
        }
        else
        {
            throw detail::BackupError("unsupported pack algorithm");
        }
        ...
        detail::cryptStage(compressed.path(), encoded.path(),
                           options.encryption, options.password, salt,
        ...
        detail::copyBytes(payload, out, info.encodedSize, options.progress,
                          "finalize", options.cancel);
        out.close();
        detail::ensure(out.good(), "cannot close final archive");
        detail::UniqueFd fd(
            ::open(finalTemp.path().c_str(), O_RDONLY | O_CLOEXEC));
        detail::ensure(fd.get() >= 0,
                       "cannot open final archive for synchronization");
        detail::ensure(::fsync(fd.get()) == 0,
        ...
        detail::ensure(
            parentFd.get() >= 0,
            "cannot open archive parent directory for synchronization");
        detail::ensure(::fsync(parentFd.get()) == 0,
                       "cannot synchronize archive parent directory; archive "
                       "already published");
        detail::ensure(::close(parentFd.release()) == 0,
                       "cannot close archive parent directory");
                                   ...
                                   const RestoreOptions& options)
{
    ...
    try
    {
        ...
        detail::withDecodedArchive(
            archive, options,
            [&](const detail::ParsedHeader&, const fs::path& packed,
                const std::vector<detail::Entry>& entries)
            {
                detail::extractEntries(packed,
                                       fs::absolute(destinationDirectory),
                                     ...
                                     const std::string& password)
{
    ...
    detail::withDecodedArchive(
        archive, options,
        [&](const detail::ParsedHeader&, const fs::path&,
            const std::vector<detail::Entry>& entries)
        {
            fs::path destination =
                fs::absolute(destinationDirectory).lexically_normal();
            std::error_code ec;
            if (fs::exists(destination, ec))
            {
                detail::ensure(!ec && fs::is_directory(destination) &&
            ...
            for (const auto& entry : entries)
            {
                ...
                {
                    preview.conflicts.push_back(entry.path);
                }
                else
                {
                    detail::ensure(errno == ENOENT,
...
ArchiveInfo BackupEngine::inspect(const std::string& archivePath)
{
    ...
    detail::ensure(detail::shaFileRange(archive, detail::kArchiveHeaderSize,
                                        header.info.encodedSize) ==
                       header.info.encodedDigest,
...
std::string toString(PackAlgorithm value)
{
    switch (value)
    {
    case PackAlgorithm::Stream:
        return "stream";
    case PackAlgorithm::Index:
        return "index";
    }
    return "unknown";
}
...
PackAlgorithm parsePackAlgorithm(const std::string& value)
{
    if (value == "stream")
    {
        return PackAlgorithm::Stream;
    }
    if (value == "index")
    {
        return PackAlgorithm::Index;
    }
