...
BackupResult BackupEngine::create(const std::string& sourceDirectory,
                                  const std::string& archivePath, const BackupOptions& options) {
    BackupResult result;
    try {
        ...
        fs::path outputParent =
            output.parent_path().empty() ? fs::current_path() : output.parent_path();
        std::error_code ec;
        ...
        detail::copyBytes(payload, out, info.encodedSize, options.progress, "finalize",
                          options.cancel);
        out.close();
...
BackupResult BackupEngine::restore(const std::string& archivePath,
    ...
    try {
        ...
        detail::withDecodedArchive(archive, options,
        ...
        [&](const detail::ParsedHeader&, const fs::path&,
            ...
            for (const auto& entry : entries) {
                preview.entries.push_back(
                    {entry.path, detail::entryTypeName(entry.type), entry.size});
                struct stat st {};
                ...
                if (lstat(target.c_str(), &st) == 0)
                    ...
                    detail::ensure(errno == ENOENT,
                                   "cannot inspect restore target: " + target.string());
            }
        });
    return preview;
}
...
ArchiveInfo BackupEngine::inspect(const std::string& archivePath) {
    ...
    detail::ensure(detail::shaFileRange(archive, detail::kArchiveHeaderSize,
                                        header.info.encodedSize) == header.info.encodedDigest,
