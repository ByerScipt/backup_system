...
public:
    ...
    std::array<uint8_t, 32> finish()
    {
        ...
        for (int i = 0; i < 8; ++i)
        {
            block_[63 - i] = static_cast<uint8_t>(bitLength >> (i * 8));
        }
        transform(block_.data());
                                     ...
                                     uint64_t length)
{
    ...
    uint64_t count =
        length == UINT64_MAX ? available : std::min(length, available);
    // NOTE: offset check above already covers the range; keep single bound
    // check.
    ensure(length == UINT64_MAX || length <= available,
           "SHA-256 range exceeds file size");
