...
UniqueFd openDirectoryAt(int rootFd, const fs::path& relative) {
    ...
    for (const auto& component : relative) {
        ...
        int fd =
            ::openat(current.get(), name.c_str(), O_RDONLY | O_DIRECTORY | O_NOFOLLOW | O_CLOEXEC);
...
class TemporaryNode {
public:
    TemporaryNode(int parentFd, std::string name) : parentFd_(parentFd), name_(std::move(name)) {}
    ~TemporaryNode() {
        if (!keep_)
...
void commitNodeAt(int parentFd, const std::string& temporary, const std::string& target,
                  bool overwrite) {
    if (overwrite) {
    ...
    if (::syscall(SYS_renameat2, parentFd, temporary.c_str(), parentFd, target.c_str(),
                  RENAME_NOREPLACE) == 0)
        return;
...
void applyMetadataAt(int parentFd, const std::string& name, const Entry& entry,
    ...
    if (entry.type != EntryType::Symlink &&
        ::fchmodat(parentFd, name.c_str(), static_cast<mode_t>(entry.mode), AT_SYMLINK_NOFOLLOW) !=
            0) {
        warnMetadata("fchmodat", displayPath);
    }
    timespec times[2]{{static_cast<time_t>(entry.atimeSec), static_cast<long>(entry.atimeNsec)},
                      {static_cast<time_t>(entry.mtimeSec), static_cast<long>(entry.mtimeNsec)}};
    if (::utimensat(parentFd, name.c_str(), times, AT_SYMLINK_NOFOLLOW) != 0 && errno != EPERM &&
        errno != EACCES && errno != ENOTSUP) {
        warnMetadata("utimensat", displayPath);
    }
}

void copyPackedFileToFd(std::ifstream& input, int outputFd, uint64_t size, const Entry& entry,
    ...
    while (completed < size) {
        ...
        size_t wanted = static_cast<size_t>(std::min<uint64_t>(buffer.size(), size - completed));
        input.read(buffer.data(), static_cast<std::streamsize>(wanted));
        ensure(static_cast<size_t>(input.gcount()) == wanted, "truncated packed file content");
        size_t written = 0;
...
void ensureDirectoryAt(int parentFd, const std::string& name, bool overwrite,
    ...
    struct stat state {};
           ...
           "restore directory already exists or is not a real directory: " + displayPath.string());
}

void createSocketNodeAt(int parentFd, const std::string& name) {
    std::string path = "/proc/self/fd/" + std::to_string(parentFd) + "/" + name;
    ensure(path.size() < sizeof(sockaddr_un::sun_path), "Unix socket restore path is too long");
    UniqueFd socketFd(::socket(AF_UNIX, SOCK_STREAM, 0));
    ensure(socketFd.get() >= 0, "cannot create Unix socket node");
    sockaddr_un address{};
    address.sun_family = AF_UNIX;
    std::strncpy(address.sun_path, path.c_str(), sizeof(address.sun_path) - 1);
    ensure(::bind(socketFd.get(), reinterpret_cast<sockaddr*>(&address), sizeof(address)) == 0,
...
void extractEntries(const fs::path& packed, const fs::path& destination, std::vector<Entry> entries,
    ...
    if (fs::exists(destination, ec)) {
        ...
        ensure(fs::create_directories(destination, ec) && !ec, "cannot create restore destination");
    }
    ...
    UniqueFd destinationFd(
        ::open(canonicalDestination.c_str(), O_RDONLY | O_DIRECTORY | O_NOFOLLOW | O_CLOEXEC));
