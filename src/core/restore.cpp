...
UniqueFd openDirectoryAt(int rootFd, const fs::path& relative)
{
    ...
    for (const auto& component : relative)
    {
        ...
        int fd = ::openat(current.get(), name.c_str(),
               ...
               "restore parent is not a real directory: " + relative.string());
        current = UniqueFd(fd);
    }
    return current;
}
...
std::string temporaryNodeName()
{
    ...
    out << ".backup-tmp-" << std::hex << random() << random()
...
public:
    ...
    ~TemporaryNode()
    {
        if (!keep_)
        {
            static_cast<void>(::unlinkat(parentFd_, name_.c_str(), 0));
        }
    }
...
template <typename Creator> std::string createTemporaryNodeAt(Creator create)
{
    for (int attempt = 0; attempt < 100; ++attempt)
    {
        std::string name = temporaryNodeName();
                  ...
                  const std::string& target, bool overwrite)
{
    if (overwrite)
    {
        ensure(::renameat(parentFd, temporary.c_str(), parentFd,
                          target.c_str()) == 0,
               "cannot atomically replace restore target: " +
...
#endif
    ensure(::linkat(parentFd, temporary.c_str(), parentFd, target.c_str(), 0) ==
               0,
           "restore target already exists or cannot be committed: " +
               std::string(std::strerror(errno)));
...
void applyMetadataAt(int parentFd, const std::string& name, const Entry& entry,
                     const fs::path& displayPath)
{
    // A successful restore includes the promised metadata. Do not silently
    // downgrade ownership or timestamp failures to a complete restore.
    ensure(::fchownat(parentFd, name.c_str(), static_cast<uid_t>(entry.uid),
                      static_cast<gid_t>(entry.gid), AT_SYMLINK_NOFOLLOW) == 0,
           "cannot restore ownership: " + displayPath.string());
    if (entry.type != EntryType::Symlink)
    {
        ensure(::fchmodat(parentFd, name.c_str(),
                          static_cast<mode_t>(entry.mode),
                          AT_SYMLINK_NOFOLLOW) == 0,
               "cannot restore permissions: " + displayPath.string());
    }
    timespec times[2]{{static_cast<time_t>(entry.atimeSec),
                       static_cast<long>(entry.atimeNsec)},
                      {static_cast<time_t>(entry.mtimeSec),
                       static_cast<long>(entry.mtimeNsec)}};
    ensure(::utimensat(parentFd, name.c_str(), times, AT_SYMLINK_NOFOLLOW) == 0,
           "cannot restore timestamps: " + displayPath.string());
}
                        ...
                        const Entry& entry, const RestoreOptions& options)
{
    ...
    while (completed < size)
    {
        ...
        {
            ssize_t count =
                ::write(outputFd, buffer.data() + written, wanted - written);
...
void ensureDirectoryAt(int parentFd, const std::string& name, bool overwrite,
                       const fs::path& displayPath)
{
    if (::mkdirat(parentFd, name.c_str(), 0700) == 0)
    {
        return;
    }
    ...
    ensure(::fstatat(parentFd, name.c_str(), &state, AT_SYMLINK_NOFOLLOW) ==
                   0 &&
                    ...
                    uint64_t& outputBytes)
{
    ...
    {
        ensure(!ec && fs::is_directory(destination) &&
    ...
    UniqueFd destinationFd(
        ::open(canonicalDestination.c_str(),
               O_RDONLY | O_DIRECTORY | O_NOFOLLOW | O_CLOEXEC));
    ensure(destinationFd.get() >= 0,
           "cannot securely open restore destination");
    ...
    outputBytes = 0;
    std::map<std::string, std::pair<dev_t, ino_t>> restoredFiles;
    for (const auto& entry : entries)
    {
        checkCancelled(options.cancel);
        if (entry.type == EntryType::Directory)
        ...
        {
            temporary = createTemporaryNodeAt(
                [&](const std::string& name) {
                    return ::symlinkat(entry.linkTarget.c_str(), parent.get(),
                                       name.c_str());
                });
        }
        ...
        else if (entry.type == EntryType::Character ||
                 entry.type == EntryType::Block)
        {
            mode_t type =
                entry.type == EntryType::Character ? S_IFCHR : S_IFBLK;
            temporary = createTemporaryNodeAt(
                [&](const std::string& name)
                {
        ...
        else if (entry.type == EntryType::Hardlink)
        {
            const auto original = restoredFiles.find(entry.linkTarget);
            ensure(original != restoredFiles.end(),
                   "hardlink target was not restored");
            UniqueFd sourceParent =
                openParentDirectory(destinationFd.get(), entry.linkTarget);
            const auto sourceName =
                fs::path(entry.linkTarget).filename().string();
            temporary = createTemporaryNodeAt(
                [&](const std::string& name)
                {
                    return ::linkat(sourceParent.get(), sourceName.c_str(),
                                    parent.get(), name.c_str(), 0);
                });
            // Validate the new link after linkat: a raced source must never be
            // committed, even if replaced between inspection and linking.
            TemporaryNode guard(parent.get(), temporary);
            struct stat linked
            {
            };
            ensure(::fstatat(parent.get(), temporary.c_str(), &linked,
                             AT_SYMLINK_NOFOLLOW) == 0 &&
                       S_ISREG(linked.st_mode) &&
                       linked.st_dev == original->second.first &&
                       linked.st_ino == original->second.second,
                   "hardlink target changed during restore");
            guard.keep();
        }
        TemporaryNode cleanup(parent.get(), temporary);
        if (entry.type == EntryType::Regular)
        {
            copyPackedFileToFd(data, regularFd.get(), entry.size, entry,
                               options);
        }
        ...
        if (entry.type == EntryType::Regular)
        {
            ...
            ensure(::fstat(regularFd.get(), &state) == 0,
                   "cannot inspect restored inode");
            restoredFiles.emplace(entry.path,
                                  std::make_pair(state.st_dev, state.st_ino));
            ensure(::fsync(regularFd.get()) == 0,
                   "cannot synchronize restored file");
            ensure(::close(regularFd.release()) == 0,
