...
Entry makeEntry(const fs::path& full, const std::string& archivePath)
{
    ...
    ensure(lstat(full.c_str(), &st) == 0,
           "cannot stat source entry: " + full.string());
    Entry e;
    e.path = archivePath;
    e.sourcePath = full;
                                 ...
                                 const BackupOptions& options)
{
    ...
    ensure(!ec && fs::is_directory(root),
    ...
    for (; it != end; it.increment(ec))
    {
        ...
        std::string archivePath =
            (fs::path(rootName) / relative).generic_string();
    ...
    // increment(ec) may report failure by returning the end iterator.
    ensure(!ec, "cannot enumerate complete source directory: " + ec.message());
    ...
    // Stable ordering ensures every hardlink refers to an earlier content
    // entry.
    std::map<std::pair<uint64_t, uint64_t>, const Entry*> originals;
    for (auto& entry : entries)
    {
        ...
        const auto original = originals.emplace(
            std::make_pair(entry.sourceDevice, entry.sourceInode), &entry);
        if (!original.second)
        {
            const Entry& target = *original.first->second;
            ensure(entry.mode == target.mode && entry.uid == target.uid &&
                       entry.gid == target.gid &&
                       entry.sourceSize == target.sourceSize &&
                       entry.mtimeSec == target.mtimeSec &&
                       entry.mtimeNsec == target.mtimeNsec,
                   "source hardlink metadata changed during scanning: " +
                       entry.path);
            // Reads can change atime without changing the file. All aliases
            // share one inode, so retain the content entry's atime snapshot.
            entry.atimeSec = target.atimeSec;
            entry.atimeNsec = target.atimeNsec;
            entry.type = EntryType::Hardlink;
            entry.linkTarget = target.path;
            inputBytes -= entry.size;
            entry.size = 0;
        }
    }
...
Entry readEntryMetadata(std::istream& in, bool withOffset)
{
    ...
    uint8_t rawType = readU8(in);
               ...
               rawType <= static_cast<uint8_t>(EntryType::Hardlink),
    ...
    if (e.type != EntryType::Symlink && e.type != EntryType::Hardlink)
    {
...
bool sameFileState(const struct stat& st, const Entry& e)
{
    if (!S_ISREG(st.st_mode) ||
        static_cast<uint64_t>(st.st_size) != e.sourceSize ||
...
void validateSourceHardlink(const Entry& entry)
{
    struct stat state
    {
    };
    ensure(::lstat(entry.sourcePath.c_str(), &state) == 0 &&
               sameFileState(state, entry),
           "source hardlink changed after scanning: " +
                    ...
                    uint64_t total, const BackupOptions& options)
{
    ...
    int fd = ::open(e.sourcePath.c_str(), flags);
           ...
           "cannot securely open source file: " + e.sourcePath.string());
    struct CloseFd
    {
        int fd;
        ~CloseFd()
    ...
    while (remaining)
    {
        ...
        ensure(got > 0, "source file changed while being read: " +
                            e.sourcePath.string());
    ...
    struct stat openedAfter
    {
               ...
               lstat(e.sourcePath.c_str(), &pathAfter) == 0 &&
               sameFileState(pathAfter, e),
...
void validateEntries(std::vector<Entry>& entries, uint64_t packedSize)
{
    ...
    std::map<std::string, const Entry*> originals;
    for (const auto& e : entries)
    {
        ensure(safeArchivePath(e.path), "unsafe archive path: " + e.path);
        ensure(paths.insert(e.path).second,
               "duplicate archive path: " + e.path);
        if (e.type == EntryType::Regular)
        {
            ensure(e.contentOffset <= packedSize &&
                       e.size <= packedSize - e.contentOffset,
                   "archive entry data is outside packed stream");
            originals.emplace(e.path, &e);
        }
        else if (e.type == EntryType::Hardlink)
        {
            const auto found = originals.find(e.linkTarget);
            ensure(safeArchivePath(e.linkTarget) && found != originals.end(),
                   "hardlink must target an earlier regular file: " + e.path);
            const Entry& target = *found->second;
            ensure(e.mode == target.mode && e.uid == target.uid &&
                       e.gid == target.gid && e.mtimeSec == target.mtimeSec &&
                       e.mtimeNsec == target.mtimeNsec &&
                       e.atimeSec == target.atimeSec &&
                       e.atimeNsec == target.atimeNsec,
                   "hardlink metadata differs from its target: " + e.path);
        }
    }
    ...
    for (const auto& entry : entries)
    {
        ...
        while (!parent.empty())
        {
            ...
            ensure(found != hierarchy.end(),
                   "archive entry has a missing parent: " + entry.path);
            ensure(found->second == EntryType::Directory,
                   "archive entry is nested below a non-directory: " +
                       entry.path);
