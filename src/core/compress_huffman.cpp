...
buildHuffmanLengths(const std::array<uint64_t, 256>& frequencies)
{
    ...
    while (queue.size() > 1)
    {
        ...
        nodes.push_back(
            {sum, -1,
             std::min(nodes[left].minimumSymbol, nodes[right].minimumSymbol),
             left, right});
        queue.push(static_cast<int>(nodes.size() - 1));
    }
...
canonicalCodes(const std::array<uint8_t, 256>& lengths)
{
    ...
    for (int symbol = 0; symbol < 256; ++symbol)
    {
        if (lengths[symbol])
        {
            ordered.emplace_back(lengths[symbol], symbol);
        }
    }
    ...
    for (size_t i = 0; i < ordered.size(); ++i)
    {
        ...
        {
            current.insert(current.end(), length - previousLength, 0);
        }
                     ...
                     const BackupOptions& options)
{
    ...
    while (first)
    {
        ...
        first.read(reinterpret_cast<char*>(buffer.data()),
                   static_cast<std::streamsize>(buffer.size()));
        size_t got = static_cast<size_t>(first.gcount());
        for (size_t i = 0; i < got; ++i)
        {
            ++frequencies[buffer[i]]; // 64-bit counter cannot overflow
        }
        counted += got;
    ...
    {
        checkCancelled(options.cancel);
        second.read(reinterpret_cast<char*>(buffer.data()),
                    static_cast<std::streamsize>(buffer.size()));
        size_t got = static_cast<size_t>(second.gcount());
        for (size_t i = 0; i < got; ++i)
        {
            writer.write(codes[buffer[i]]);
        }
        completed += got;
                       ...
                       uint64_t expectedSize, const RestoreOptions& options)
{
    ...
    ensure(magic == std::array<char, 4>{{'H', 'U', 'F', '1'}},
    ...
    std::array<uint8_t, 256> lengths{};
    readExact(in, lengths.data(), lengths.size());
    auto codes = canonicalCodes(lengths);
    std::ofstream out(output, std::ios::binary | std::ios::trunc);
    ...
    for (int symbol = 0; symbol < 256; ++symbol)
    {
        ...
        ensure(trie[node].symbol < 0 && trie[node].child[0] < 0 &&
                   trie[node].child[1] < 0,
               "duplicate or non-prefix-free Huffman table");
        trie[node].symbol = symbol;
    }
    ensure((originalSize == 0 && symbolCount == 0) ||
               (originalSize > 0 && symbolCount > 0),
    ...
    while (produced < originalSize)
    {
        ...
        if (trie[node].symbol >= 0)
        {
            ...
            if (outputBuffer.size() == kBufferSize)
            {
                ...
                outputBuffer.clear();
                report(options.progress, "decompress-huffman", produced,
                       originalSize);
            }
        }
    }
