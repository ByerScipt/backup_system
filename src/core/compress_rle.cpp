...
void compressStage(const fs::path& input, const fs::path& output, CompressionAlgorithm algorithm,
    ...
    case CompressionAlgorithm::None:
        copyFileStage(input, output, "compress-copy", options.progress, options.cancel);
        break;
...
void decompressStage(const fs::path& input, const fs::path& output, CompressionAlgorithm algorithm,
    ...
    case CompressionAlgorithm::None:
        ...
        copyFileStage(input, output, "decompress-copy", options.progress, options.cancel);
        break;
    case CompressionAlgorithm::Rle:
...
void rleDecompress(const fs::path& input, const fs::path& output, uint64_t expectedSize,
    ...
    ensure(originalSize == expectedSize, "RLE declared output size does not match archive header");
    std::ofstream out(output, std::ios::binary | std::ios::trunc);
