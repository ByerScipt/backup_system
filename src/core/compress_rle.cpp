                   ...
                   CompressionAlgorithm algorithm, const BackupOptions& options)
{
    ...
    case CompressionAlgorithm::None:
        copyFileStage(input, output, "compress-copy", options.progress,
                      options.cancel);
        break;
    case CompressionAlgorithm::Rle:
        rleCompress(input, output, options);
        break;
    case CompressionAlgorithm::Huffman:
        huffmanCompress(input, output, options);
        break;
    default:
        throw BackupError("unsupported compression algorithm");
    }
}
                     ...
                     const RestoreOptions& options)
{
    ...
    case CompressionAlgorithm::None:
            ...
            fileSizeChecked(input) == expectedSize,
            "decoded packed size mismatch (wrong password or damaged archive)");
        copyFileStage(input, output, "decompress-copy", options.progress,
                      options.cancel);
        break;
    case CompressionAlgorithm::Rle:
        rleDecompress(input, output, expectedSize, options);
        break;
    case CompressionAlgorithm::Huffman:
        huffmanDecompress(input, output, expectedSize, options);
        break;
    default:
        throw BackupError("unsupported compression algorithm");
    }
                   ...
                   uint64_t expectedSize, const RestoreOptions& options)
{
    ...
    ensure(magic == std::array<char, 4>{{'R', 'L', 'E', '1'}},
           "invalid RLE header");
    ...
    while (produced < originalSize)
    {
        ...
        ensure(count <= expectedSize - produced,
               "RLE block exceeds output size limit");
        if (control & 0x80u)
        {
