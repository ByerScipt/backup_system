...
std::vector<Entry> readStreamEntries(std::ifstream& in, uint64_t size) {
    ...
    for (uint32_t i = 0; i < count; ++i) {
        ...
        ensure(e.size <= size - std::min<uint64_t>(size, e.contentOffset),
               "truncated sequential entry");
...
std::vector<Entry> readIndexEntries(std::ifstream& in, uint64_t size) {
    ...
    readExact(in, endMagic.data(), endMagic.size());
    ...
    uint32_t trailerCount = readU32(in);
               ...
               centralSize <= size - 32 - centralOffset,
    ...
    std::array<char, 8> centralMagic{};
    readExact(in, centralMagic.data(), centralMagic.size());
    ensure(centralMagic == kCentralMagic, "invalid central directory magic");
    uint32_t count = readU32(in);
    ensure(count == trailerCount && count > 0 && count <= kMaxEntries,
