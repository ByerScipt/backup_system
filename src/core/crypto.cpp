...
// Self-contained (no third-party code) so the course extension keeps full
// credit.
...
void chacha20Block(const std::array<uint8_t, 32>& key,
                   ...
                   uint8_t out[64])
{
    static constexpr uint32_t kConstants[4] = {0x61707865u, 0x3320646eu,
    ...
    for (int i = 0; i < 8; ++i)
    {
        state[4 + i] = load32le(key.data() + i * 4);
    }
    state[12] = counter;
    for (int i = 0; i < 3; ++i)
    {
        state[13 + i] = load32le(nonce.data() + i * 4);
    }
    uint32_t working[16];
...
{
    static constexpr uint8_t kRcon[8] = {0x00, 0x01, 0x02, 0x04,
                                         0x08, 0x10, 0x20, 0x40};
...
inline void aesAddRoundKey(uint8_t state[16], const uint8_t roundKeys[240],
                           int round)
{
    for (int i = 0; i < 16; ++i)
    {
...
inline void aesMixColumns(uint8_t state[16])
{
    for (int col = 0; col < 4; ++col)
    {
        ...
        state[col * 4] =
            static_cast<uint8_t>(xtime(a0) ^ (xtime(a1) ^ a1) ^ a2 ^ a3);
        state[col * 4 + 1] =
...
void aes256EncryptBlock(const uint8_t roundKeys[240], const uint8_t in[16],
                        uint8_t out[16])
{
    uint8_t state[16];
...
void chacha20Xor(const std::array<uint8_t, 32>& key,
                 const std::array<uint8_t, 12>& nonce, uint64_t position,
...
// XOR with AES-256-CTR keystream; the 16-byte salt is the initial counter
// block.
void aes256CtrXor(const std::array<uint8_t, 32>& key,
                  const std::array<uint8_t, 16>& counter, uint64_t position,
                  uint8_t* data, size_t size)
{
    ...
    while (done < size)
    {
        ...
        for (size_t i = 0; i < take; ++i)
        {
            data[done + i] ^= keystream[offset + i];
        }
        done += take;
        ++blockIndex;
        offset = 0;
