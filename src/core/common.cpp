               ...
               std::atomic_bool* cancel)
{
    ...
    while (done < count)
    {
        ...
        ensure(static_cast<size_t>(in.gcount()) == wanted,
               "truncated input while copying data");
...
void commitFileNoReplace(const fs::path& temporary, const fs::path& output)
{
    ...
    {
        throw BackupError(
            "cannot atomically commit output without replacement: " +
            std::string(std::strerror(errno)));
    }
#endif
    ensure(::link(temporary.c_str(), output.c_str()) == 0,
           "cannot commit output without replacement: " +
               std::string(std::strerror(errno)));
