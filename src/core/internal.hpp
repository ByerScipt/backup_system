...
inline constexpr uint16_t kArchiveVersion = 2;
...
inline constexpr std::array<char, 8> kStreamMagic{
    {'S', 'T', 'R', 'M', 'P', 'K', '1', '\0'}};
inline constexpr std::array<char, 8> kIndexMagic{
    {'I', 'N', 'D', 'X', 'P', 'K', '1', '\0'}};
inline constexpr std::array<char, 8> kCentralMagic{
    {'C', 'D', 'I', 'R', 'V', '1', '\0', '\0'}};
inline constexpr std::array<char, 8> kIndexEndMagic{
    {'I', 'D', 'X', 'E', 'N', 'D', '1', '\0'}};
...
struct Entry
{
    ...
    uint64_t sourceSize = 0; // Scan-time size, including hardlink aliases.
};
...
using DecodedArchiveConsumer = std::function<void(
    const ParsedHeader&, const fs::path&, const std::vector<Entry>&)>;
